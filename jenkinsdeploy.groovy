pipeline {
    ...
    stages {
      ...
      stage('apply') {
          ...
          steps{
             slackSend channel: "#terraform", color: 'warning', message: "*[${action}]* *${JOB_NAME}* building *${envid}* *${targetString}* needs approval. <${env.BUILD_URL}|Open>"
            script {
              // TODO: notification to slack channel
              def approval = input(submitterParameter: 'submitter', message: 'Should we continue?')
              slackSend channel: "#terraform", color: 'good', message: "*[${action}]* *${JOB_NAME}* building *${envid}* *${targetString}* approved. <${env.BUILD_URL}|Open>"
            }
            dir("roles/${appid}"){
              script {
